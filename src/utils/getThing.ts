...
import {Command, CommandHandler, isTextChannelLike} from '../index.js';
	...
	switch (dataType) {
		...
		case DataType.CHANNEL:
			return client?.channels.cache.get(text) ?? message?.mentions.channels.first() ??
				client?.channels.cache.find(c => c instanceof GuildChannel &&
					c.name.toLowerCase().includes((text as string).toLowerCase()) && text.toString().length > 1 || c instanceof DMChannel &&
					c.recipient.username.toLowerCase().includes((text as string).toLowerCase()) && text.toString().length > 2 ||
					c.toString() === text.toString().replace(/<#(\d{17,19})>/, '<@$1>') || false) ?? client?.channels.resolve(text) ?? null;
		case DataType.EMOTE:
			// noinspection JSRemoveUnnecessaryParentheses
			return client?.emojis.cache.get(text) ??
				client?.emojis.cache.find(e => (e.name?.toLowerCase().includes((text as string).toLowerCase()) && (text as string).length >
					1) ?? false) ?? Util.resolvePartialEmoji(text) ?? null;

		case DataType.GUILD:
			return client?.guilds.cache.get(text) ??
				client?.guilds.cache.find(g => g.name.toLowerCase().includes((text as string).toLowerCase()) && (text as string).length >
					1) ?? client?.guilds.resolve(text) ?? message?.mentions.guild ?? null;
		case DataType.MEMBER:
			return message?.guild?.members.cache.get(text) ?? message?.mentions?.members?.first() ??
				message?.guild?.members.cache.find(m => (m.displayName.toLowerCase().includes((text as string).toLowerCase()) ||
						m.user.username.toLowerCase().includes((text as string).toLowerCase())) && (text as string).length > 1 ||
					m.toString() === text.toString().replace(/<@!?(\d{17,19})>/, '<@$1>')) ?? message?.guild?.members.resolve(text) ?? null;
		...
		case DataType.TEXT_CHANNEL:
			const result =
				...
				client?.channels.cache.find(c => isTextChannelLike(c) && c.name.toLowerCase().includes((text as string).toLowerCase()) &&
		...
		case DataType.ROLE:
			return message?.guild?.roles.cache.get(text) ?? message?.mentions.roles.first() ??
				message?.guild?.roles.cache.find(r => r.name.toLowerCase().includes((text as string).toLowerCase()) &&
					(text as string).length > 1 || r.toString() === text.toString().replace(/<@&(\d{17,19})>/, '<@$1>')) ??
				message?.guild?.roles.resolve(text) ?? null;

		case DataType.USER:
			return client?.users.cache.get(text) ??
				client?.users.cache.find(u => u.username.toLowerCase() === (text as string).toLowerCase() || u.toString() ===
					text.toString().replace(/<@!?(\d{17,19})>/, '<@$1>')) ?? message?.mentions?.users.first() ??
				client?.users.resolve(text) ?? null;
		default:
			return null;
	}
}
